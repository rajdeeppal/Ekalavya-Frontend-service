...
const RoleManagement = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios.get('http://localhost:61002/roleAudit')
      .then((response) => {
  ...
  const handleSearch = () => {
    if (selectedRole) {
      setLoading(true);
      axios.get(`http://localhost:61002/roleAudit/searchRole/${selectedRole}`, {
        headers: {
  ...
  const handleRevoke = (userId, roleId) => {
    setLoading(true);
    axios.post(`http://localhost:61002/roleAudit/revokeRole`, null, {
      params: {
  ...
  const handleAssignRole = (userId) => {
    ...
    if (newRole) {
      setLoading(true);
      axios.post(`http://localhost:61002/roleAudit/changeRole`, null, {
        params: {
          userId: userId,   // Pass userId as a request parameter
